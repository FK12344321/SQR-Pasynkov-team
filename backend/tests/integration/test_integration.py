...
PASSWORD = ''.join(random.choices(string.ascii_uppercase + string.digits, k=30))
...
def compare_activity(activity, response_body):
    ...
    assert datetime.strptime(response_body['start_date'], "%Y-%m-%dT%H:%M:%S") == \
        datetime.strptime(activity['start_date'], "%Y-%m-%dT%H:%M:%S.%fZ")
    assert datetime.strptime(response_body['end_date'], "%Y-%m-%dT%H:%M:%S") == \
        datetime.strptime(activity['end_date'], "%Y-%m-%dT%H:%M:%S.%fZ")
...
def test_integrati():
    ...
    for activity_type in ACTIVITY_TYPES: 
        headers = {'Authorization': 'Bearer ' + access_token}
        response = client.get(f'/activity?page_index=1&page_size=300&activity_type={activity_type}', headers=headers)
        relevant_activities = list(filter(lambda activity: activity['activity_type'] == activity_type, activities))
    ...
    for year in range(2000, 2020):
        ...
        response = client.get(f'/activity?page_index=1&page_size=300&start_date={start_date}&end_date={end_date}', headers=headers)
